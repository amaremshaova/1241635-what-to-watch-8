...
function Tabs(): JSX.Element
{
  ...
  if (starringTextRef.current !== null && film) {
    starringTextRef.current.innerHTML = film.starring.map((star)=> ` <br/> ${star}`).join();
  }
    ...
    return(
      <div className="film-card__desc">
        ...
        <div className="film-card__text film-card__row"
          style={tab === TabValues.Overview ? {display : 'block'} : {display : 'none'}}
        >
        ...
        <div className="film-card__reviews film-card__row"
          style={tab === TabValues.Review ? {display : 'flex'} : {display : 'none'}}
        >
          <div className="film-card__reviews-col">
            {reviews.slice(0, 3).map((review) => <Review review={review} key={review.id + review.date}/>)}
          </div>
          <div className="film-card__reviews-col">
            {reviews.slice(3, 6).map((review) => <Review review = {review} key={review.id + review.date}/>)}
          </div>
        </div>
      </div>
    );
